...
```
marimo/ingest_app.py   (501)  Ingestion UI: upload/drop sources, run ingest,
                                  scan dir, delete source, view log. Calls
                                  domain.ingestion.pipeline / batch + deletion.
marimo/read_app.py     (356)  Reading + RAG chat + save-to-wiki:
...
```
marimo/widgets/delete_confirm.py  (179)  DeleteConfirmWidget — anywidget

...
The LLM Wiki local index is designed entirely around the concept of **derived state**. The database acts as a high-performance local cache and index for workspace files (such as Markdown files, PDFs, and assets).
* **Non-destructive Loss:** If this SQLite database file is deleted or corrupted, it can be fully rebuilt by scanning the source filesystem workspace.

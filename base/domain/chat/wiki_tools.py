...
def file_to_wiki(
    ...
    """Save a chat-generated synthesis as a new wiki concept page, or merge into an existing one.

    Use this when you have synthesised new knowledge worth preserving —
    a comparison table, a summary of a conversation, an analysis you produced.

    Args:
        ctx: The runtime context containing system dependencies.
        title: Human-readable title for the concept (e.g. "Quantitative Easing Comparison").
        content: Full markdown content for the page.
        category: "concept" (default) or "summary". Determines the destination directory.

    Returns:
        Confirmation message with the created/updated path and any automatic repair notes.
    """
    ...
    # 1. Retrieve paths from agent context
    db_path = ctx.deps
    workspace = _workspace(db_path)

    # 2. Extract LLM configuration settings from the project configuration
    ...
    # Initialize the client used to communicate with the OpenAI-compatible language model
    client = OpenAI(base_url=base_url, api_key=api_key)

    # 3. Determine the correct subdirectory based on the page's category
    dir_path = "/wiki/concepts/" if category == "concept" else "/wiki/summaries/"

    # 4. Generate a safe, URL-friendly filename (slug) from the human-readable title (e.g. "My Title" -> "my-title")
    slug = make_wiki_slug(title)

    try:
        # 5. Check if a wiki page with this slug already exists in the file system
        ...
        if existing:
            # Overwrite the existing page with the newly merged/structured content
            result = create_page(
                db_path, workspace, dir_path, slug, title, structured,
                tags=[category], overwrite=True,
            )
            page_path = result["path"]
            action = "Updated"
        else:
            # Create a completely new page on disk
            result = create_page(
                db_path, workspace, dir_path, slug, title, structured,
                tags=[category], overwrite=False,
            )
            page_path = result["path"]
            action = "Created"

        # 8. Synchronize the citation and reference graph inside the database
        doc_id_row = None
        try:
            from domain.tools.db import get_connection
            with get_connection(db_path) as conn:
                # Query the database to find the internal auto-incrementing ID for this file's path
                row = conn.execute(
            ...
            # Silently ignore database retrieval issues to prevent saving from crashing entirely
            pass

        # If a matching database document ID was found, parse the page and record links/references
        if doc_id_row:
            update_references(db_path, doc_id_row, structured, dir_path)

        # 9. Extract the first heading or line as a short summary (up to 80 chars) to update the global index.md
        summary = structured.strip().splitlines()[0].lstrip("# ").strip()[:80]
        update_index(workspace, page_path, summary, category + "s")

        # 10. Run a fast "lint and repair" cycle on the newly saved page to fix broken links, formatting, etc.
        msg = f"{action} wiki page: {page_path}"
        ...
        # Log the full exception stack trace for developers and return an informative message
        logger.error("file_to_wiki failed for '%s': %s", title, exc)

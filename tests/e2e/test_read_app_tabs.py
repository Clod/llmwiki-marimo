...
async def test_chat_strict_mode_refuses_off_corpus_question(chat_page: Page) -> None:
    ...
    # Anchor on the unique tail of the label, not on "Strict mode": the
    # pre-retrieval checkbox beside it reads "…(supersedes strict mode)", and
    # get_by_text is case-insensitive, so the shorter phrase matches both.
    toggle = chat_page.get_by_role("checkbox", name="answer only from wiki sources")
